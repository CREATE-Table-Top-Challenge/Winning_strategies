...
def hausdorff_distance(set1, set2):
    ...
    Parameters:
    - set1: numpy array of points from the first set.
    - set2: numpy array of points from the second set.
    
    Returns:
    - The Hausdorff distance as a float.
    """
    forward = directed_hausdorff(set1, set2)[0]
    backward = directed_hausdorff(set2, set1)[0]
    return max(forward, backward)

def extract_points(binary_mask):
    """
    Extract the coordinates of the boundary points from a binary mask.
    
    Parameters:
    - binary_mask: 2D numpy array of type bool.
    
    Returns:
    - numpy array of points where each point is [x, y].
    """
