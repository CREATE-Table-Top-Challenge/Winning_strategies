...
def get_arguments():
    ...
    default_save_path = "/home/jovyan/Desktop/Central_Line_Challenge/_models_results/Tool_Detection"
    data_csv_location = "/home/jovyan/Desktop/Training_Data/Training_Data.csv"
    ...
    parser.add_argument(
        ...
        type=list,
        default=["AN04", "MS01"],
        help='Folder(s) of the data to be used for validation'
    )
    parser.add_argument(
        '--val_iou_threshold',
        type=float,
...
def prepareDataK_Fold(datacsv, labelName,class_mapping,saveDir,val_fold,include_blank,balance):
    config = {}
    ...
    for learning_set in sets:
        ...
        if not os.path.exists(sample_file_path):
            inverted_class_mapping = invert_class_mapping(class_mapping)
            writeLabelTextFiles(data, labelName, saveDir,inverted_class_mapping,include_blank,balance)
...
def fetch_backbone(bb):
    assert bb.startswith('yolov') and bb[-1] in '568'
    if bb == 'yolov5':
        return YOLOv5()
    elif bb == 'yolov6':
        return YOLOv6()
...
def main(args):
    if args.save_location == "":
        ...
        with open(os.path.join(saveDir,"config.yaml"),"w") as f:
            yaml.dump(config,f)
        if args.use_kfold == "":
            dataPath = prepareData(dataCSVFile, 
                                   args.label_name, 
                                   class_mapping, saveDir, args.val_percentage,
                                   args.include_blank, args.balance)
        else:
            dataPath = prepareDataK_Fold(dataCSVFile, 
                                         args.label_name, 
                                         class_mapping, saveDir, args.val_fold,
                                         args.include_blank, args.balance)
        model_config = setAugmentationParams(saveDir)
        updateModelConfig(args,saveDir)
        #yolo = YOLOv5()
        yolo = fetch_backbone(args.backbone)
