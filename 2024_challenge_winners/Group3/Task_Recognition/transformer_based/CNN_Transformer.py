...
class CNN_Transformer:
    ...
    def loadModel(self, modelFolder, modelName=None):
        ...
        self.loadTransformerModel(modelFolder)
    ...
    def loadTransformerModel(self,modelFolder):
        self.transformer_model = WorkflowTransformer(num_features=self.num_features,num_classes = self.num_classes)
        checkpoint = torch.load(os.path.join(modelFolder,"transformer.pth"))
        self.transformer_model.load_state_dict(checkpoint["model"],strict=True)
        try:
            self.transformer_model.cuda(self.device)
        except AttributeError:
            pass
    ...
    def predict(self,image):
        ...
        with torch.no_grad():
            ...
            taskPrediction = self.transformer_model(torch.from_numpy(expanded_sequence).float().cuda(self.device))
    ...
    def createTransformerModel(self,num_features,num_classes):
        self.transformer_model = WorkflowTransformer(num_features,num_classes)
        return self.transformer_model
...
class WorkflowTransformer(nn.Module):
    def __init__(self, num_features, num_classes, dropout=0.1):
        ...
        self.embedding = nn.Linear(num_features, self.hidden_size)
        #self.pos_encoder = PositionalEncoding(d_model=self.hidden_size, dropout=dropout)
        encoder_layers = nn.TransformerEncoderLayer(d_model=self.hidden_size, nhead=8)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=6)
        self.linear_1 = nn.Linear(self.hidden_size, 64)
        self.relu1 = nn.ReLU()
