...
def unpackZipFiles(dataLocation,datasetType):
    if datasetType == "Train":
        ...
        for i in range(1,9):
            print("Extracting data from: Unlabelled_Part{}.zip".format(i))
            extractionDir = os.path.join(dataLocation, "Unlabelled_Part{}".format(i))
            if not os.path.exists(extractionDir):
                os.mkdir(extractionDir)
            zipFile = os.path.join(dataLocation, "Unlabelled_Part{}.zip".format(i))
            try:
                shutil.unpack_archive(zipFile, extractionDir)
            except shutil.ReadError:
                #zipRef = zipfile.ZipFile(zipFile, 'r')
                #zipRef.extractall(extractionDir)
                print(f'{zipFile} is not a .zip file')
...
def checkAllPresent(dataLocation,datasetType):
    ...
    if datasetType == "Train":
        ...
        for i in range(1,9):
            zipFile = os.path.join(dataLocation, "Unlabelled_Part{}.zip".format(i))
            if not os.path.exists(zipFile):
                missingFiles.append("Unlabelled_Part{}.zip".format(i))
...
def createDataset():
    ...
    if datasetType == "Train":
        ...
        if not os.path.exists(dataSetLocation):
            os.mkdir(dataSetLocation)
        for i in range(1, 9):
            os.listdir(baseLocation)
            dataFolder = os.path.join(baseLocation, "Unlabelled_Part{}".format(i))
            for videoDir in os.listdir(dataFolder):
                moveFilesToMainDirectory(os.path.join(dataFolder, videoDir), dataSetLocation)
            shutil.rmtree(dataFolder)
            print("Removed empty directory {}".format(dataFolder))
