...
def get_arguments():
    ...
    parser.add_argument(
        '--evolve', 
        action='store_true', 
        help='Enable hyperparameter evolution'
    )
...
def prepareData(datacsv, labelName,class_mapping,saveDir,val_percentage,include_blank):
    ...
    train_annotators = ['AN01', 'AN02', 'AN03', 'AN04', 'MS01', 'MS02', 'MS03', 'MS04']
    val_annotators = ['AN05', 'MS05']
...
def updateModelConfig(hyperparams, saveDir):
    # This function now accepts 'hyperparams' which can be a namespace or dictionary
    config_path = os.path.join(saveDir, "model_config.yaml")
    # Load existing configuration or initialize an empty one if not present
    try:
        with open(config_path, 'r') as f:
    ...
    # Update configuration: map 'hyperparams' directly if it's a dict or via attributes if it's an argparse.Namespace
    params_to_update = ['epochs', 'optimizer', 'patience', 'lr0', 'lrf', 'freeze', 'close_mosaic', 
                        'box', 'cls', 'dfl', 'batch', 'imgsz', 'iou', 'conf']
    for param in params_to_update:
        value = getattr(hyperparams, param, None) if isinstance(hyperparams, argparse.Namespace) else hyperparams.get(param, None)
        if value is not None:  # This ensures only provided values are updated
            config[param] = value
...
######################################
def evolve_hyperparameters(data, config, saveDir, generations=10, population_size=10, top_k=2):
    param_space = {
        'lr0': (1e-6, 1e-2),
    ...
    population = [generate_random_hyperparams(param_space) for _ in range(population_size)]
    fields = ['generation', 'hyperparameters', 'score']
    
    # Open CSV file for writing
    csv_file_path = os.path.join(saveDir, 'train/hyperparam_tuning/evolution_history.csv')
    with open(csv_file_path, 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fields)
        writer.writeheader()

        for generation in range(generations):
            scores = [evaluate_hyperparameters(hp, data, config) for hp in population]
            top_indices = np.argsort(scores)[-top_k:]
            elite = [population[i] for i in top_indices]

            # Logging details to CSV
            for index, score in enumerate(scores):
                writer.writerow({
                    'generation': generation,
                    'hyperparameters': population[index],
                    'score': score
                })
            ...
            while len(new_population) < population_size:
                if random.random() < 0.5:
                    ...
                    child = crossover_hyperparameters(parent1, parent2)
                    print(f"Generation {generation}: Crossover between {parent1} and {parent2} to form {child}")
                else:
                    parent = random.choice(elite)
                    child = mutate_hyperparameters(parent, param_space)
                    print(f"Generation {generation}: Mutated {parent} to form {child}")
...
# def evolve_hyperparameters(data, config, generations=10, population_size=10, top_k=2):
#     """Evolve hyperparameters using a genetic algorithm."""
...
#         top_indices = np.argsort(scores)[-top_k:]  # Maximize the score
#         elite = [population[i] for i in top_indices]
...
#     # Return the best set of hyperparameters after all generations
#     best_index = np.argmax(scores)
#     return population[best_index]

def generate_random_hyperparams(space):
    """Generate a random set of hyperparameters from the defined space."""
    return {k: np.random.uniform(*v) if isinstance(v, tuple) else random.choice(v) for k, v in space.items()}

def evaluate_hyperparameters(hyperparams, data, config):
    """A function to train the model and return some evaluation metric."""
    # # This function should integrate the model's training and validation process
    # # For now, let's assume it returns a random performance score
    # return np.random.random()
    
    # Set the hyperparameters
    saveDir = config['save_location']
    args = config['args']  # Assuming all necessary args are passed here as a dictionary or Namespace

    # Prepare the data path based on the hyperparameters
    dataPath = prepareData(data, args.label_name, config['class_mapping'], saveDir, args.val_percentage, args.include_blank)

    # Update model config with current hyperparameters
    model_config = setAugmentationParams(saveDir)
    updateModelConfig(hyperparams, saveDir)  # Assuming this function can now handle hyperparameter dicts

    yolo = YOLOv5()

    # Path to save weights adjusted for hyperparameter evaluation to prevent overwriting main weights
    weights_path = os.path.join(saveDir, "train/hyperparam_tuning/weights/best.pt")
    if not os.path.exists(weights_path):
        ...
        model.train(data=dataPath,
                    ...
                    verbose=True,
                    cache=False,
                    project=os.path.join(saveDir, "train/hyperparam_tuning"),
                    exist_ok=True,
                    amp=False,
                    cfg=model_config)
    yolo.loadModel(os.path.join(saveDir, "train/hyperparam_tuning"))  # Make sure to load from the specific hyperparam tuning directory
    model = yolo.model
    ...
    metrics = model.val(split="val",
                        device=args.device,
                        cfg=model_config)
    # Extract the desired metric, e.g., mAP at IoU=0.5
    map_metric = metrics['mAP_0.5'] if 'mAP_0.5' in metrics else 0

    # Optionally, clear up memory and model weights if they won't be used again
    del model, metrics, yolo
    gc.collect()
    os.remove(weights_path)  # Remove specific hyperparameter tuning weights to save disk space

    return map_metric


def crossover_hyperparameters(hp1, hp2):
...
def mutate_hyperparameters(hyperparams, param_space):
    """Mutate a set of hyperparameters."""
    mutation = hyperparams.copy()
    param_to_mutate = random.choice(list(param_space.keys()))
    if isinstance(param_space[param_to_mutate], tuple):
        mutation[param_to_mutate] = np.random.uniform(*param_space[param_to_mutate])
    else:
        mutation[param_to_mutate] = random.choice(param_space[param_to_mutate])
    return mutation
...
def main(args):
    if args.save_location == "":
        ...
        with open(os.path.join(saveDir,"config.yaml"),"w") as f:
            yaml.dump(config,f)
        if args.evolve:
            #Perform hyperparam evolution
            best_config = evolve_hyperparameters(dataCSVFile, config, saveDir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser('YOLOv5 training script with hyperparameter evolution option', parents=[get_arguments()])
