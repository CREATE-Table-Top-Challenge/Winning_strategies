...
def get_arguments():
    ...
    parser.add_argument(
        '--batch_size',
        type=int,
        default=16, #NOTE original was 16
    ...
    parser.add_argument(
        "--optimizer",
        type=str,
        default='auto', #NOTE changed to auto
    ...
    parser.add_argument(
        ...
        #maybe test sizes of 384, 416, 512
        type=int,
        default=416, #NOTE original was 224
        help='Size of input images as integer'
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=1, #NOTE original 1
...
def setAugmentationParams(saveDir):
    ...
    config['flipud'] = 0.5 # image flip up-down (probability)
    config['fliplr']=0.5 # image flip left-right (probability)
    config['label_smoothing']=0.05 # label smoothing (fraction)
    config['hsv_h']=0.1 # image HSV-Hue augmentation (fraction)
    config['hsv_s']=0.1 # image HSV-Saturation augmentation (fraction)
    config['hsv_v']=0.1 # image HSV-Value augmentation (fraction)
    config['degrees']=10 # image rotation (+/- deg)
    config['translate']=0.1 # image translation (+/- fraction)
    config['scale']=0.1 # image scale (+/- gain)
    config['shear']=5 # image shear (+/- deg)
    config['perspective']=0.0005 # image perspective (+/- fraction), range 0-0.001
    config['mosaic']=0.5 # image mosaic (probability)
    config['mixup']=0.2 # image mixup (probability)
    config['copy_paste']=0.05 # segment copy-paste (probability)
