...
def get_arguments():
    ...
    parser.add_argument(
        '--data_csv_file',
        type=str,
        default='dataset/Training_Data/Training_Data.csv',
...
def prepareData(datacsv, labelName,class_mapping,saveDir,val_percentage,include_blank):
    ...
    for learning_set in sets:
        ...
        if learning_set == "Validation":
            config["val"] = 'Validation.txt' #sample_file_path
        else:
            config[learning_set.lower()] = 'Train.txt' #sample_file_path
...
def setAugmentationParams(saveDir):
    ...
    config['flipud'] = 0.00856 # image flip up-down (probability)
    config['fliplr']=0.5 # image flip left-right (probability)
    config['label_smoothing']=0.0 # label smoothing (fraction)
    config['hsv_h']=0.0138 # image HSV-Hue augmentation (fraction)
    config['hsv_s']=0.664 # image HSV-Saturation augmentation (fraction)
    config['hsv_v']=0.464 # image HSV-Value augmentation (fraction)
    config['degrees']=0.0 # image rotation (+/- deg)
    config['translate']=0.245 # image translation (+/- fraction)
    config['scale']=0.898 # image scale (+/- gain)
    config['shear']=0.0 # image shear (+/- deg)
    config['perspective']=0.0 # image perspective (+/- fraction), range 0-0.001
    config['mosaic']=0.1 # image mosaic (probability)
    config['mixup']=0.243 # image mixup (probability)
