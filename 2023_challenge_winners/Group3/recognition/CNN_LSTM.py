...
class CNN_LSTM():
    ...
    def loadCNNLSTMModel(self, modelFolder):
        with open(os.path.join(modelFolder,"labels.txt"), "r") as f:
            self.labels = f.readlines()
        self.labels = numpy.array([x.replace("\n","") for x in self.labels])

        structureFileName = 'cnn_lstm.json'
        weightsFileName = 'cnn_lstm.h5'
        modelFolder = modelFolder.replace("'","")
        with open(os.path.join(modelFolder, structureFileName), "r") as modelStructureFile:
            JSONModel = modelStructureFile.read()
        model = model_from_json(JSONModel)
    ...
    def infer(self, image):
        ...
        #resized = cv2.resize(image, (299, 299))  #InceptionV3
        normImage = cv2.normalize(resized, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
        input_img = numpy.expand_dims(numpy.expand_dims(normImage, axis=0), axis=0)

        taskClassification = self.cnn_lstm.predict(input_img, verbose=0)
        labelIndex = numpy.argmax(taskClassification)
    ...
    def build_CNN_model(self, hp):
        ...
        #model.add(InceptionV3(weights='imagenet', include_top=False, input_shape=imageSize))
        model.add(layers.Dense(hp.Int("dense_units", min_value=256, max_value=768, step=256), activation='relu'))
        # model.add(layers.Dense(512,activation='relu'))
        model.add(layers.Dense(10, activation='softmax'))

        # Compile model
        hp_lr = hp.Choice("learning_rate", values=[1e-3, 1e-4, 1e-5])
        model.compile(optimizer=tensorflow.keras.optimizers.Adam(learning_rate=hp_lr), loss="categorical_crossentropy", metrics=['accuracy'])
        return model
    ...
    def build_LSTM_model(self, hp):
        input = layers.Input(shape=(50, 10))
        bLSTM = layers.Bidirectional(layers.LSTM(10, return_sequences=False))(input)
        # r1 = layers.Dense(hp.Int("dense_units", min_value=32, max_value=96, step=32), activation='relu')(bLSTM)
        # r2 = layers.Dense(hp.Int("dense_units", min_value=16, max_value=48, step=16), activation='relu')(r1)
        r1 = layers.Dense(10, activation='relu')(bLSTM)
        r2 = layers.Dense(10, activation='relu')(r1)
        out = layers.Dense(10, activation='softmax')(r2)
        model = tensorflow.keras.models.Model(inputs=input, outputs=out)

        hp_lr = hp.Choice("learning_rate", values=[1e-3, 1e-4, 1e-5])
        adam = tensorflow.keras.optimizers.Adam(learning_rate=hp_lr)
        model.compile(optimizer=adam, loss='categorical_crossentropy', metrics=['accuracy'])
        return model
    
    def createCNNLSTMModel(self, image_size, sequence_len, num_classes):
        model = tensorflow.keras.models.Sequential()
        model.add(layers.Input(shape=(sequence_len, *image_size)))
    ...
    def saveCNNLSTM(self, trainedCNNLSTM, saveLocation):
        JSONmodel = trainedCNNLSTM.to_json()
        structureFilename = 'cnn_lstm.json'
        with open(os.path.join(saveLocation, structureFileName), "w") as modelStructureFile:
            modelStructureFile.write(JSONmodel)
