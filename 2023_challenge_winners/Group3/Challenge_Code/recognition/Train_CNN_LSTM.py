...
import CNN_LSTM
from CNNLSTMSequence import CNNSequence, LSTMSequence, CNNLSTMSequence
...
class Train_CNN_LSTM:
    ...
    def train(self):
        if FLAGS.save_location == "":
            ...
            self.tune_hp = FLAGS.tune_hp
            self.load_cnn_weights = FLAGS.load_cnn_weights
            ...
            cnnTrainDataSet = CNNSequence(self.dataCSVFile, TrainIndexes, 256, taskLabelName)
            cnnValDataSet = CNNSequence(self.dataCSVFile, ValIndexes, 256, taskLabelName)
            ...
            modelCheckPointCallback = ModelCheckpoint(os.path.join(self.saveLocation,'resnet50.h5'), verbose=0,monitor='val_accuracy', mode='max', save_weights_only = True,save_best_only=True)

            cnn_tuner = kt.Hyperband(network.build_CNN_model,
                                    objective="val_accuracy",
                                    max_epochs=10,
                                    factor=3,
                                    overwrite=False,
                                    directory=self.saveLocation,
                                    project_name="create_challenge")

            # Hyperparameter tuning
            if self.tune_hp:
                cnn_tuner.search(cnnTrainDataSet, validation_data=cnnValDataSet, epochs=self.numEpochs, callbacks=[earlyStoppingCallback])
            best_cnn_hps = cnn_tuner.get_best_hyperparameters()[0]
            print(best_cnn_hps.values)
            best_cnn_model = cnn_tuner.get_best_models()[0]
            best_cnn_model.summary()

            if self.load_cnn_weights:
                ...
                cnnModel = cnn_tuner.hypermodel.build(best_cnn_hps)
            ...
            lstmTrainDataSet = LSTMSequence(self.dataCSVFile, inputs, lstmTrainSequences, cnnModel, 256, taskLabelName)
            print("Training images loaded")
            lstmValDataSet = LSTMSequence(self.dataCSVFile, inputs, lstmValSequences, cnnModel, 256, taskLabelName)
            print("Validation images loaded")

            modelCheckPointCallback = ModelCheckpoint(os.path.join(self.saveLocation, 'single_LSTM.h5'), verbose=0,
            ...
            lstm_tuner = kt.Hyperband(network.build_LSTM_model,
                                     objective="val_accuracy",
                                     max_epochs=10,
                                     factor=3,
                                     directory=self.saveLocation,
                                     project_name="create_challenge_lstm")
            
            # Hyperparameter tuning
            lstm_tuner.search(lstmTrainDataSet, validation_data=lstmValDataSet, epochs=self.numLSTMEpochs, callbacks=[earlyStoppingCallback])
            best_lstm_hps = lstm_tuner.get_best_hyperparameters()[0]
            print(best_lstm_hps.values)
            best_lstm_model = lstm_tuner.get_best_models()[0]
            best_lstm_model.summary()

            lstmModel = lstm_tuner.hypermodel.build(best_lstm_hps)
            ...
            # all_seq = self.splitDatasetIntoSequences(self.dataCSVFile.index, sequenceLength=self.sequenceLength,downsampling=self.downsampling)
            # train_seq, val_seq = sklearn.model_selection.train_test_split(all_seq, test_size=self.validation_percentage)

            # if self.balanceLSTM:
            #     train_seq = self.getBalancedSequences(train_seq)
            #     val_seq = self.getBalancedSequences(val_seq)

            # self.labelValues = numpy.array(sorted(self.dataCSVFile[taskLabelName].unique()))
            # numpy.savetxt(os.path.join(self.saveLocation, "labels.txt"), self.labelValues, fmt='%s', delimiter=',')

            # train_dataset = CNNLSTMSequence(self.dataCSVFile, train_seq, self.batch_size, taskLabelName)
            # print("Training images loaded")
            # val_dataset = CNNLSTMSequence(self.dataCSVFile, val_seq, self.batch_size, taskLabelName)
            # print("Validation images loaded")

            # modelCheckPointCallback = ModelCheckpoint(os.path.join(self.saveLocation, 'cnn_lstm.h5'), verbose=1,
            #                                           monitor='val_accuracy', mode='max', save_weights_only=True,
            #                                           save_best_only=True)
            # earlyStoppingCallback = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=5)

            # cnn_lstm = network.createCNNLSTMModel((224, 224, 3), self.sequenceLength, len(self.labelValues))
            # cnn_lstm.compile(optimizer=self.lstm_optimizer, loss=self.loss_Function, metrics=self.metrics, run_eagerly=False)

            # history = cnn_lstm.fit(x=train_dataset,
            #                        validation_data=val_dataset,
            #                        epochs=self.numEpochs,callbacks=[modelCheckPointCallback,earlyStoppingCallback])
            # cnn_lstm.load_weights(os.path.join(self.saveLocation, 'cnn_lstm.h5'))

            # self.saveTrainingInfo(self.saveLocation, history.history, "cnn_lstm")
            # self.saveTrainingPlot(self.saveLocation, history.history, "loss","cnn_lstm")
            # for metric in self.metrics:
            #     self.saveTrainingPlot(self.saveLocation, history.history, metric, "cnn_lstm")
            
            # network.saveCNNLSTM(cnn_lstm, self.saveLocation)
