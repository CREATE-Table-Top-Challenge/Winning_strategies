...
class CNNLSTMSequence(Sequence):
    def __init__(self, datacsv, sequences, batchSize, labelName, shuffle=True, transforms=False):
        self.inputs = numpy.array([os.path.join(row["Folder"], row["FileName"]) for idx, row in datacsv.iterrows()])
        self.targets = numpy.array([datacsv[labelName][x] for x in datacsv.index])
        self.sequences = sequences
        self.batchSize = batchSize
        self.labelName = labelName
        ...
        self.labels = numpy.array(sorted(datacsv[self.labelName].unique()))
        targetSequences = self.readLabelSequences(datacsv.index)
        self.targets = targetSequences
        self.on_epoch_end()
    
    def on_epoch_end(self):
        if self.shuffle:
            shuffledInputs, shuffledTargets = shuffle(self.sequences, self.targets)
            self.sequences = shuffledInputs
            self.targets = shuffledTargets

    def __len__(self):
        # author Rebecca Hisey
        length = len(self.sequences) / self.batchSize
        length = math.ceil(length)
        return length
    ...
    def readLabelSequences(self, indexes):
        allLabels = []
        smallestIndex = indexes[0]
        for sequence in self.sequences:
            label = self.getSequenceLabels(sequence,smallestIndex)
            allLabels.append(label)
        return numpy.array(allLabels)
    
    def readImages(self, indices):
        image_seq = numpy.zeros((0, 224, 224, 3))
        for idx in indices:
            image = cv2.imread(self.inputs[idx])
            resized_image = cv2.resize(image, (224, 224))
            normImg = cv2.normalize(resized_image, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
            image_seq = numpy.concatenate((image_seq, numpy.expand_dims(normImg, axis=0)))
            del image
            del resized_image
        return image_seq
    ...
    def __getitem__(self, index):
        startIndex = index * self.batchSize
        indexOfNextBatch = (index + 1) * self.batchSize
        inputBatch = numpy.array([self.readImages(x) for x in self.sequences[startIndex:indexOfNextBatch]])
        outputBatch = numpy.array([x for x in self.targets[startIndex:indexOfNextBatch]])
        if inputBatch.shape == (0,) or outputBatch.shape == (0,):
            print(inputBatch.shape)
            print(self.sequences[startIndex: indexOfNextBatch])
